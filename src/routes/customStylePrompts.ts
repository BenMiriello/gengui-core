  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (prompt.length > MAX_CUSTOM_STYLE_PROMPT_LENGTH) {
        res.status(400).json({
          error: {
            message: `Prompt must be ${MAX_CUSTOM_STYLE_PROMPT_LENGTH} characters or less`,
            code: 'INVALID_INPUT',
          },
        });
        return;
      ...
      if (existingPrompts.length >= MAX_CUSTOM_STYLE_PROMPTS_PER_USER) {
        res.status(400).json({
          error: {
            message: `Maximum ${MAX_CUSTOM_STYLE_PROMPTS_PER_USER} custom prompts allowed`,
