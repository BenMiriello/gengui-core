...
(isFalkorDBAvailable ? describe : describe.skip)('Graph: Query Operations', () => {
  ...
  describe('query (raw Cypher)', () => {
    ...
    test('executes query with parameters', async () => {
      const result = await graphService.query('RETURN $value as value', { value: 42 });
  ...
  describe('findNodes', () => {
    ...
    test('finds nodes with property filter', async () => {
      ...
      const result = await graphService.findNodes('Character', { name: 'Alice' });
      expect(result.data).toHaveLength(1);
    });
  });
  ...
  describe('narrative threads', () => {
    test('creates narrative thread', async () => {
      const threadId = await graphService.createNarrativeThread(testDocumentId, testUserId, {
        name: 'Main Plot',
        isPrimary: true,
        eventNames: [],
      });
